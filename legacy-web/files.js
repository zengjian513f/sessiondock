...
(() => {
  ...
  const labels = {mkdir:'新建文件夹', 'new-file':'新建文件', rename:'重命名', copy:'复制', move:'移动', delete:trashDelete ? '删除（移入回收目录）' : '永久删除', trash:'历史删除', restore:'历史还原', purge:'历史清理', compress:'压缩', extract:'解压', bundle:'打包下载', upload:'上传'};
  let data = null, controller, selected = new Set(), anchor = -1, loading = false;
  let clipboard = restore('clipboard', null);
  ...
  const historyKey = 'history:' + context.toString();
  ...
  function actionEnabled(action) {
    ...
    const backend = {new:'mkdir', upload:'upload', cut:'move', copy:'copy', paste:clipboard?.action, rename:'rename', delete:'delete', compress:'compress', extract:'extract'}[action];
    if (backend && !supports(backend)) return false;
    if (action === 'copy') return !!count && !loading;
    if (action === 'download') return !!count && !loading && ((writable && supports('bundle')) || (count === 1 && chosen()[0]?.kind === 'file'));
  ...
  function render(result) {
    ...
    for (const [index,name] of ['根目录', ...segments].entries()) {
      ...
      const link = element('a', name); navigation(link, reachable(path || '/') ? (path || '/') : null);
  ...
  async function load() {
    ...
    try {
      ...
      if (controller !== request) return;
      if (previous) {
        // A refused navigation (outside the bound root, missing directory)
        // keeps the last good listing: restore crumbs, address, paging and
        // rows, and point the URL back at it so a refresh does not re-fail.
        loading = false; render(previous);
        history.replaceState(history.state, '', pageURL(previous.path, previous.offset));
  ...
  const conflictField = () => ({name:'conflict', label:'遇到同名项目', value:'error', options:[['error','停止该项并提示'],['keep','保留两份（自动编号）'],['skip','跳过'],['replace','覆盖原项目']]
    .filter(([value]) => !writeCaps || !Array.isArray(writeCaps.conflicts) || writeCaps.conflicts.includes(value))});
  ...
  async function perform(action) {
    ...
    try {
      ...
      if (action === 'copy' || action === 'cut') {
        ...
        store('clipboard',clipboard); selectionChanged(); status(`已${action === 'cut' ? '剪切' : '复制'} ${paths.length} 项`); return;
      }
      ...
      if (action === 'delete') {
        if (trashDelete) {
          if (await ask('删除',`把选中的 ${paths.length} 个项目移入 SessionDock 回收目录（服务端私有状态目录，不在项目目录内）？不会永久删除。\n${paths.join('\n')}`,[],'删除')) await submit({action,paths});
          return;
        }
  ...
  function renderTasks(jobs) {
    ...
    for (const job of jobs) {
      ...
      if (supports('retry') && ['failed','cancelled','interrupted'].includes(job.state) && !['trash','restore','purge'].includes(job.action)) button('重试',async () => {
  ...
  async function sendUpload(job,file) {
    ...
    try {
      ...
      do {
        const chunk = file.slice(offset,offset + UPLOAD_CHUNK);
  ...
  async function startUploads(files,destination) {
    ...
    // The picker can be opened without the toolbar button (drop, script,
    // browser restore); the current directory is then the destination.
    destination = destination || data?.path || '';
    if (!destination) { status('请先打开一个可写目录再上传', true); return; }
  ...
  for (const id of ['sort','view','hidden']) {
    ...
    $(id).onchange = () => { preferences[id] = id === 'hidden' ? $(id).checked : $(id).value; store('view',preferences); if (id === 'view' && data) renderEntries(); else navigate(data?.path || '',0); };
  }
  function orderLabel() { $('order').textContent = preferences.order === 'asc' ? '升序 ↑' : '降序 ↓'; }
  orderLabel(); $('order').onclick = () => { preferences.order = preferences.order === 'asc' ? 'desc' : 'asc'; orderLabel(); store('view',preferences); navigate(data?.path || '',0); };
  ...
  $('upload-input').onchange = () => { const picked = [...$('upload-input').files]; $('upload-input').value = ''; startUploads(picked, uploadDestination || data?.path || ''); };
  ...
  addEventListener('storage',event => { if (event.key === storageKey('clipboard')) { clipboard = restore('clipboard',null); selectionChanged(); } });
