...
(() => {
  ...
  const prefix = SessionDockCapabilities.namespace || (hub ? `sessiondock.hub.${root}.` : 'sessiondock.');
  function apply() {
    let choice = 'ubuntu';
    try { choice = JSON.parse(SessionDockCapabilities.stored('font', prefix)) || choice; } catch {}
