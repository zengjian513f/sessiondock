...
(async () => {
  ...
  try {
    ...
    if (context.get('open') === '1') {
      ...
      if (!target && !result.resolved?.[context.get('ref')]) {
        const failure = result.errors?.find(item => item.ref === context.get('ref'));
        throw new Error(failure?.error || '文件不存在、未在会话中提及，或有多个同名文件；请使用完整路径。');
      }
      context.delete('open');
    ...
    if (!response.ok) {
      // An explicitly rejected PDF preview is still downloadable as inert
      // octet-stream bytes. Keep the recovery action promised by the error.
      if (info.code === 'file_invalid_pdf') {
        const download = document.getElementById('file-download');
        download.href = api({download:1}); download.hidden = false;
      }
      throw new Error(info.error || '无法打开文件');
    }
