...
function renderMachineSettings() {
  ...
  if (machineOrderTimer) return;      // 键盘排序尚未提交：别用服务端的旧顺序盖掉它
  closeMachinePalette();
...
let machineOrderVersion = 0;   // 每次用户排序 +1；旧保存的重画不覆盖更新的顺序
...
function startMachineDrag(event, row, grip) {
  ...
  const finish = e => {
    ...
    if (after.join() !== dragged.before.join()) { machineOrderVersion++; void saveMachineOrder(after); }
  };
...
function moveMachineRow(row, step) {
  ...
  machineOrderVersion++;
  // 顺序在按键当下就定下来：防抖期间若有一次周期性的机器列表刷新重画了行，
  // 定时器再去读 DOM 会拿到服务端的旧顺序，把用户刚按出来的顺序提交丢。
  const ids = [...rows.children].map(r => r.dataset.machine);
  clearTimeout(machineOrderTimer);   // 连按几下只提交最后的顺序
  machineOrderTimer = setTimeout(() => {
    machineOrderTimer = 0;
    void saveMachineOrder(ids);
  }, 400);
}

async function saveMachineOrder(ids) {
  // 一次保存的"重载 + 重画"只在没有更新的用户排序时进行：拖动后的重载若晚于
  // 紧接着的键盘排序，重画会把新顺序盖回旧的，随后的防抖保存再把旧顺序提交上去。
  const version = machineOrderVersion;
  const current = () => version === machineOrderVersion;
  try {
