...
| `SESSIONDOCK_SEARCH_FOLD_BYTES` | `search_fold_bytes` | no (default) | Byte cap of the resident case-folded copies of cached bodies that the search prefilter reads, least recently used evicted first (`SESSIONDOCK_SEARCH_FOLD_BYTES`, default 128 MiB); SESSIONDOCK_SEARCH_FOLD_BYTES must be... | no | [performance.md](performance.md), [read-model.md](read-model.md) |
