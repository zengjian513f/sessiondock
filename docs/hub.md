...
`http://<字面 IP>[:端口]`（无路径/凭据/query/fragment，`/` 结尾允许，端口 1–65535，
缺省 80，IPv6 用 `[…]`），且 IP 落在允许网络内（CIDR 列表，严格形式：主机位非零即拒绝；缺省只有
`127.0.0.0/8,::1/128`——Python 的缺省还多一个私网 /24，Rust 侧要求把 WireGuard 网段显式
配置，缺省不放行）。随后请求节点
...
**只有 `http://`**：Python 也接受 `https://`，Rust 客户端不带 TLS（Hub↔节点走 WireGuard
私网，字面 IP 也没有证书可校验），`validate_url` 直接拒绝 `https://`。连同上面的缺省网络，这是本包仅有的两处有意差异。
...
**依赖决定：手写 HTTP/1.1，不引入 `hyper-util` legacy client。** Python 只用
...
socket 与已预读字节供 101 升级双向转发）、`Client::connect()` + `Pending::send()`
...
| --- | --- | --- |
| `SESSIONDOCK_NODE_BIND` | 第二监听地址（WireGuard 接口 IP:端口） | 必须是一个具体接口地址（拒绝 `0.0.0.0` / `::`），与 `SESSIONDOCK_BIND` 不同 |
| `SESSIONDOCK_NODE_TOKEN_FILE` | 节点凭据文件（Python `--node-token-file`） | 存在的绝对路径普通文件，无 group/other 权限位；去首尾空白后 `[A-Za-z0-9._~+/=-]{32,256}`（`NodeToken::load`） |
| `SESSIONDOCK_NODE_ID_FILE` | 节点身份文件（Python `--node-id-file`） | 绝对路径；已存在则必须是 32 位小写十六进制（只读），不存在则父目录必须存在，启动时由 `identity::node_id` 以 `O_EXCL` 0600 生成 |
| `SESSIONDOCK_NODE_PEERS` | 允许的来源网段（严格 CIDR，逗号分隔） | 主机位非零即拒绝；空表拒绝；没有缺省值 |

两份文件都不得落在前端、原生读根、host、state、delivery、lifecycle、launcher、audit、
trash、Codex 索引、Grok active 文件或文件根之内，也不能是同一个文件。`--check-config`
额外打印 `node_bind` / `node_token_file` / `node_id_file` / `node_peers`（未配置为 `(unset)`）。
...
- `delete(registry, client, body)`：`uids` 先 `split` 分组（首次出现顺序），空 →
  `没有选中任何会话`；逐机顺序 `POST /api/sessions/delete {uids}`（`client.timeout`），
...
| `SESSIONDOCK_HUB_NODES` | `hub-nodes.json` 注册表（必填） | 绝对路径无 `..`；已存在则必须是无 group/other 权限位的普通文件；不存在则父目录必须存在（首次注册时落盘） |
| `SESSIONDOCK_HUB_CACHE_DIR` | 离线会话快照目录 | 绝对路径无 `..`；缺省是注册表旁的 `hub-cache`；与注册表/审计目录互不包含 |
| `SESSIONDOCK_HUB_NETWORKS` | 可注册的节点 IP/CIDR（严格） | 主机位非零即拒；缺省 `127.0.0.0/8,::1/128`（私网 WireGuard 段须显式配） |
| `SESSIONDOCK_WEB_DIR` | 前端快照（hub 模式） | 存在的目录；与注册表/缓存/审计互不包含 |
| `SESSIONDOCK_AUDIT_DIR` | `hub.node.*.changed` 记录（可选） | 与节点的 audit 校验同规则（0700、无符号链祖先）；未配置则不记显示/顺序变更 |
...
hostname `AgentHub`、能力声明 `hub_capabilities()`（`backend:rust, hub:true,
...
`cross-origin write rejected`）、URI ≤ 16 KiB；响应加 `nosniff`/`Referrer-Policy`/API `no-store`。
...
`hyper_util::rt::TokioIo` 拿到浏览器连接，和 H2 客户端 `Body::into_raw` 交还的节点 socket 做
...
- `cargo test -p sessiondock --lib -- hub::proxy hub_config`（proxy 9 例 + hub_config 2 例）：
  ...
  hub_config loopback bind 与私有注册表文件、缓存/审计与前端目录互不包含。
