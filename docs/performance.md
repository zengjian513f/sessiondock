...
起因：Orion 上一个 1.59 GB 的 Codex 会话（10,046 行；229 行 ≥ 2 MiB、564 行在
64 KiB–2 MiB 之间，都是截图 base64，合计 1.5 GB）以及从它分叉出来的十个子会话
（`history_base` 指向它的 1.58 GB 前缀）。改前在 Orion（Ryzen 9 9950X，部署二进制
c0d2f28，隔离实例、只读真实根、页缓存热）：

| 打开（`window=1`，冷） | 改前耗时 | 改前 RSS | 改后（3135a8f）耗时 | 改后 RSS |
| --- | ---: | ---: | ---: | ---: |
| 1.59 GB 主会话 | 5.77 s | 62 → 1517 MB | 1.09 s | 52 MB |
| 分叉子会话（8.9 MB 叶 + 1.58 GB 继承前缀） | 5.96 s | 1600 MB | 1.04 s | 92 MB |
| 95 MB 叶 + 1.94 GB 链 | 7.45 s（15,685 条） | 1827 MB | 1.99 s | 200 MB |

热打开、`append=1` 空增量与第一页历史在改前改后都 ≤ 11 ms；改后依次打开三个会话
空闲后 RSS 202 MB（改前 1831 MB）。
...
| 文件 | 阶段 | 改前 | 改后 |
| --- | --- | ---: | ---: |
| 合成 Codex 1.37 GB / 14,062 行 | 只读字节 | 0.49 s | 0.49 s |
| | 物理索引（分行 + 校验点） | 2.16 s | 0.37 s |
| | 索引 + 记录解码 | 6.97 s | 1.46 s |
| | `parse_candidate` 全程 | 7.60 s | 1.56 s |
| | 视图驻留的图片 | 572 MB | 0 |
| | 同 stamp 再解析（追加路径） | 7.42 s | 0.49 s |
| 真实 Codex 217 MB / 36,124 行（截图会话） | 物理索引 | 0.43 s | 0.07 s |
| | 索引 + 记录解码 | 1.92 s | 1.09 s |
| | `parse_candidate` 全程 | 2.05 s | 1.24 s |
| | 视图驻留的图片 | 18 MB | 0 |
| 真实 Claude 49 MB / 23,763 行（文本会话） | 物理索引 | 0.086 s | 0.016 s |
| | 索引 + 记录解码 | 0.72 s | 0.46 s |
| | `parse_candidate` 全程 | 0.92 s | 0.68 s |

文本会话的下限现在是 `serde_json` 本身：同一 49 MB 文件全部行只经 `serde_json`
解成 `Value` 也要 0.47 s（`benchmark_serde_baseline`），且树的估重 225 MB（文件的
4.6 倍，`preserve_order` 的 IndexMap 与逐字符串分配），所以它仍超出 64 MiB 的 AST
复用预算，追加时整文件重解码。下一步若要再降，就得不为每条记录建完整的树：按
来源跳过投影从不读取的顶层字段（这份 Claude 文件里 `snapshot` 占 28%、
`toolUseResult` 占 14%），或把物理索引与浅投影按文件版本落盘。
