...
chmodded to `0700` on Unix, matching Python's audit store setup. Files are
created with mode `0600`.

The wire shape is `{page_id|_page_id, uid, _build, _trace_id, events:[...]}`.
The route keeps Python's 4 MiB request-body limit and maximum of 100 events per
...
Writers use a nonblocking queue with Python's 20,000-item capacity. A full
...
Python's 14-day retention window are deleted.
