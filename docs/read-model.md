...
- **折叠副本与预筛**（`search/fold.rs`、`search/prefilter.rs`，2026-09-15）：
  ...
  每个查询先在折叠副本上跑预筛：字面/全词查询要求折叠 needle 出现；正则查询从
...
- **匹配**：候选来自索引（按 `updated` 倒序，`?debug_run=` 视图筛过；同一
  ...
  **全词**字面查询取字面量的每个出现，再检查前后邻字符是否在 `[\p{L}\p{N}_]`
  （`regex-syntax` 同一张表），与 `(?<![\p{L}\p{N}_])(?:lit)(?![\p{L}\p{N}_])`
  逐位置等价，但不再让回溯引擎在出现之间逐字符扫描（lookbehind 使引擎失去
  字面量预扫，原来 19 MB 要 3 s CPU）；`regex=1` 的查询仍由 `fancy-regex`
