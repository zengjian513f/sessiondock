...
| brand_names_check | `python3 tests/brand_names_check.py` | Scans every tracked non-Markdown text file—source, tests, configuration, static assets and frozen reference—with no compatibility whitelist; also verifies the SessionDock manifest, install metadata, service-worker cache/offline text, page/runtime titles and `<namespace>files-*` preference keys. | git checkout | <1s |
...
| fork_messages_parity | `python3 tests/fork_messages_parity.py --python-source PATH --binary target/release/sessiondock` | Batch 35: `/api/messages` (user/assistant role+text) vs Python `CodexAdapter.read` for legacy forks, rewind-past-fork and plain threads (grok-4.6 headless draft, reviewed) | binary, python-source | n/a |
...
| send_browser | `python3 tests/send_browser.py` | Claude reliable send through the real legacy composer against the fake Claude CLI (desktop + 390 px, batch 31); a second page's send and composer Esc refused with the owner while the console is held, the 390 px Esc written with no console open. | Chromium, ptyhost | n/a |
