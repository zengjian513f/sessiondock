...
| deploy_dry_run | `python3 tests/deploy_dry_run.py` | Offline regression for `deploy/deploy.py`: `build --web-only` into a stage, `push --dry-run` against a temporary local prefix (PLANNED / SKIPPED / UNSUPPORTED rows, plan lines, JSON report, fixture untouched), unknown target and refused rollback dir; no ssh, no cargo (grok-4.6 headless draft, reviewed) | git checkout | 2s |
| deploy_native_handlers | `python3 tests/deploy_native_handlers.py` | Offline command-sequence pins for the `macos-node` and `windows-node` deploy handlers with a recording fake Shell (stage/backup/swap/restart/verify/rollback, rendered `.cmd` templates); the Windows path has no real-machine run yet | none | <1s |
