...
| side_drag_browser | `python3 tests/side_drag_browser.py --binary target/release/sessiondock` | Sidebar divider under real CDP touch drag/cancel at 814x380 and mouse drag/dblclick at desktop; desktop row text selection survives the synthetic click and defers list refresh until selection clears; width stored under `sessiondock.` (Python `side_drag_e2e` port, batch 37 F) | binary, Chromium | 6s |
