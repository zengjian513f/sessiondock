...
impl Disk {
    ...
    pub(super) fn open(path: &Path, initialize: bool) -> Result<Self, Error> {
        ...
        let lock = options.open(&lock_path).map_err(|error| {
            if initialize && error.kind() == io::ErrorKind::AlreadyExists {
                ...
                io_error("open stable delivery lock", error)
            }
        })?;
        let metadata = lock
            .metadata()
        ...
        match lock.try_lock() {
            ...
            Err(_) => return Err(Error::Io("acquire OS delivery lock", io::ErrorKind::Other)),
        }
        let disk = Self {
            #[cfg(unix)]
    ...
    fn check_owner(&self) -> Result<(), Error> {
        ...
        let metadata = fs::symlink_metadata(self.directory.join(LOCK_FILENAME))
            .map_err(|error| io_error("check delivery lock", error))?;
        private_file(&metadata)?;
        let handle = self
            .lock
            .metadata()
    ...
    pub(super) fn read(&self) -> Result<Option<Vec<u8>>, Error> {
        ...
        let before = match fs::symlink_metadata(&path) {
            ...
            Err(error) => return Err(io_error("inspect delivery ledger", error)),
        };
        private_file(&before)?;
        ...
        let mut file =
            File::open(&path).map_err(|error| io_error("open delivery ledger", error))?;
        let opened = file
            .metadata()
        ...
        (&mut file)
            ...
            .map_err(|error| io_error("read delivery ledger", error))?;
        if bytes.len() > MAX_BYTES {
            return Err(Error::Limit);
        }
        let after = fs::symlink_metadata(&path)
            .map_err(|error| io_error("restamp delivery ledger", error))?;
        private_file(&after)?;
        let handle = file
            .metadata()
    ...
    fn create_temp(&self) -> Result<Temporary, Error> {
        for _ in 0..8 {
            ...
            match private_options().create_new(true).open(&path) {
                ...
                Err(error) => return Err(io_error("create unique delivery temp", error)),
            }
        }
        Err(Error::Io(
            "delivery temp collision",
