        ...
        with host(root, instance, uid=uid) as (process, record), sync_playwright() as playwright:
            ...
            try:
                with isolated_server(corpus, BINARY, host_dir=root / "host") as (base, opener):
                    ...
                    assert live["enabled"] is True and live["known"] is True and live["partial"] is False, live
                    assert live["tmux_uids"] == [uid]
                    ...
                    def unrelated_stay_inactive(active):
                        ...
                        assert page.locator("#livecount").get_attribute("data-unknown") != "true"
                        for other in others:
                    ...
                    assert "visible" not in (page.locator("#dlive").get_attribute("class") or "").split()
                ...
                browser.close()
    print(f"PASS live browser: verified running managed instance, exit evidence {exit_evidence}, unrelated sessions inactive on desktop/mobile, live API polling, native fixtures unchanged")
