#!/usr/bin/env python3
"""One-time preference migration from the Python `agenthub.*` keys (batch 44 WP-F).

A same-origin deployment that replaced the Python page still holds every
preference under `agenthub.<key>` (files pages: bare `agenthub-files-<key>`).
...
2. copy each read value to `sessiondock.<key>` exactly once and never write the
   `agenthub.*` keys again (a later change lands only under the new prefix);
...
4. on `files.html`, key preferences as `sessiondock.files-<key>` and migrate
   both older spellings (`sessiondock.agenthub-files-<key>`, then the bare
   Python `agenthub-files-<key>`);
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="sessiondock-prefs-migration-") as temporary:
        ...
        seed = {
            "agenthub.theme": '"dark"', "agenthub.font": '"cascadia"', "agenthub.width": "420",
            "agenthub.sideCollapsed": "false", "agenthub.nest": "true", "agenthub.view": '"date"',
            "agenthub.compactTurns": "false", "agenthub.off": '["grok"]', "agenthub.cacheMb": "64",
            "agenthub.toolIcons": '"boss"', "agenthub.unread": json.dumps([[unread, {"count": 3}]]),
            "agenthub.sel": json.dumps(target), "agenthub.nodesOff": '["stale-node"]',
            "agenthub.settingsTab": '"appearance"',
            # 3: an existing Rust key wins over the Python one.
            "agenthub.mobilePage": '"detail"', "sessiondock.mobilePage": '"list"',
            # 4: files pages — the pre-rename Rust spelling beats the bare Python key;
            #    the clipboard only exists under the Python key.
            "agenthub-files-view": json.dumps({"view": "list", "order": "desc", "sort": "size", "hidden": True}),
            "sessiondock.agenthub-files-view": json.dumps({"view": "grid", "order": "desc", "sort": "size", "hidden": True}),
            "agenthub-files-clipboard": json.dumps({"action": "copy", "node": None, "paths": [str(files / "notes.md")]}),
        }
        with isolated_server(corpus, args.binary, file_roots=(files,)) as (base, _), sync_playwright() as playwright:
            ...
            try:
                ...
                for key in ("theme", "font", "width", "nest", "view", "compactTurns", "off", "cacheMb",
                    ...
                    assert dump["agenthub." + key] == seed["agenthub." + key], key
                for key in ("theme", "font", "width", "nest", "view", "compactTurns", "off", "cacheMb", "toolIcons", "nodesOff"):
                    assert dump["sessiondock." + key] == seed["agenthub." + key], (key, dump["sessiondock." + key])
                assert dump["sessiondock.mobilePage"] == '"list"' and dump["agenthub.mobilePage"] == '"detail"'
                ...
                assert after["sessiondock.nest"] == "false" and after["agenthub.nest"] == "true", after
                page.locator("#settings").click()
                page.locator("#setting-theme").select_option("light")
                page.wait_for_function("document.documentElement.dataset.theme === 'light'")
                after = page.evaluate(LS_DUMP)
                assert after["sessiondock.theme"] == '"light"' and after["agenthub.theme"] == '"dark"', after
                page.keyboard.press("Escape")
                # The Python keys are the same set as seeded: nothing was added or removed there.
                assert {k: v for k, v in after.items() if k.startswith("agenthub.")} == {k: v for k, v in seed.items() if k.startswith("agenthub.")}
                ...
                assert dump["agenthub-files-view"] == seed["agenthub-files-view"]
                assert dump["agenthub-files-clipboard"] == seed["agenthub-files-clipboard"]
                assert dump["sessiondock.agenthub-files-view"] == seed["sessiondock.agenthub-files-view"]
                ...
                dump = manager.evaluate(LS_DUMP)
                assert dump["agenthub-files-view"] == seed["agenthub-files-view"], "Python files key never written"
                assert dump["sessiondock.agenthub-files-view"] == seed["sessiondock.agenthub-files-view"], "old Rust spelling never written"
                assert not [k for k in dump if k.startswith("sessiondock.agenthub-files-") and k != "sessiondock.agenthub-files-view"], dump
                context.close()
                ...
                dump = page.evaluate(LS_DUMP)
                assert not [k for k in dump if k.startswith("agenthub")], dump
        ...
        print("PASS prefs migration browser: seeded agenthub.* preferences applied and copied once to sessiondock.*, "
