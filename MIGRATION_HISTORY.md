...
- 左栏文字选区修复（2026-09-13）：桌面端拖选会话标题、目录、元信息或分组名时，拖选产生的合成 `click` 不再打开/折叠行；从鼠标按下到非空 Selection 消失期间延迟整栏重画，解除后从最新内存状态补画并保持滚动位置。`side_drag_browser` 增加拖拽期间 1 ms 重画竞态、误导航、选区持久与解除后追平断言；专项 Chromium、66 项 legacy Node 契约和生产真实列表只读检查通过。通用 legacy/nest-tree Chromium 在本机并行全量 Rust 测试期间均卡于 Playwright 控件稳定性等待，未到断言阶段，不记作通过。首次只部署 Lyra 节点，用户复验未生效后确认实际入口是中央 Hub；随后 Hub、Lyra、Cygnus、Pavo、Cetus 五端均部署同一前端摘要并健康。Hub 聚合实页的 1 ms 重画竞态复验通过；重启前后 Lyra 13、Pavo 2、Cetus 2 个受管宿主 PID 保持不变，Cygnus 原为 0，并在各端保留部署前静态文件回滚副本。
- Cetus 受管会话蓝点修复（2026-09-13）：无外部进程扫描的平台此前只把运行中的 ptyhost 会话写入 `/api/live.uids`，遗漏 `tmux_uids`，导致可 attach 的 Windows 受管会话显示绿点。现在受管运行集合同时初始化两者；有进程扫描时仍由完整合并结果覆盖。Linux 隔离树状态单测 2 项、runtime 4 项、runtime_live 2 项及 Clippy/fmt 通过；Cetus 生产源码快照原生单测 2 项、runtime 4 项、runtime_live 1 项与隔离生产配置预检通过。首次误用非生产源码目录构建的 EXE 因配置预检失败已立即回滚，未进入稳定运行；随后从精确生产快照重新构建并部署。目标 RVC 会话在节点响应中同时进入 `uids`/`tmux_uids` 且保持 running，两个既有 ptyhost PID 未变化；旧 EXE 保存在私有回滚目录。
- Pavo 新建会话 attach 延迟修复（2026-09-13）：22:56 创建的 `m1.05pq` Codex 宿主与子进程始终存活；浏览器点击后先整栏重画，并在首次 `ensureTerm` 同步初始化 WebGL，软件渲染环境实测阻塞 40–44 s，期间没有发出 claim。已有 pending 行改为原位选中，新建 pending view 先用 DOM renderer 连接；legacy 契约 68 项通过。真实 Hub 注入修复前 claim 约 41–49 s，部署后 claim 0.752 s、attach WebSocket 0.775 s（HTTP 200、无页面异常）；只更新并重启中央 Hub，Pavo 节点和目标 ptyhost PID 2471718 未重启，回滚副本为 `backup-pending-attach-20260913-234021`。
